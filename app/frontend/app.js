...
function App() {
	const [message, set_message] = use_state('Connecting to Firebase...')
	const [db_status, set_db_status] = use_state('')
	...
	const seed_and_read = async () => {
		try {
			...
			const snapshot = await get_docs(collection_ref)
			
			if (snapshot.empty) {
				await add_doc(collection_ref, {
					text: 'Hello World from Firestore!'
				})
				set_db_status('First run — document created in Firestore.')
			}
			
			const updated_snapshot = await get_docs(collection_ref)
