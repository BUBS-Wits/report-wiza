...
const normalise_request = (id, data) => {
    ...
    return {
        ...
        sa_ward:      data.sa_ward != null ? String(data.sa_ward) : null,  // ← always a string
        municipality: data.sa_m_name ?? 'Unknown Municipality',
        sa_m_name:    data.sa_m_name ?? 'Unknown Municipality',            // ← keep both keys
...
export const fetchPublicDashboardData = async () => {
	...
	snapshot.forEach((doc_snap) => {
		...
		if (normalised.status === 'resolved' || normalised.status === 'closed') {
