...
export const build_category_stats = (all_requests) => {
  return REQUEST_CATEGORIES.map((category) => {
    ...
    const pending     = cat_requests.filter((r) => PENDING_STATUSES.includes(r.status))
    const in_progress = cat_requests.filter((r) => IN_PROGRESS_STATUSES.includes(r.status))
    const resolved    = cat_requests.filter((r) => RESOLVED_STATUSES.includes(r.status))
...
export const compute_summary = (stats) => {
  ...
  const worst_backlog = [...stats].sort((a, b) => b.pending - a.pending)[0] ?? null
