...
function FitMapToRequests({ requests }) {
  ...
  useEffect(() => {
    ...
    const bounds = L.latLngBounds(validRequests.map(r => {
...
function PublicDashboard() {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        ...
        const open = all.filter(req => req.status?.toLowerCase() !== 'resolved')
        const resolved = all.filter(req => req.status?.toLowerCase() === 'resolved')
  ...
  const wardsAffected = new Set(allRequests.map(req => req.ward).filter(Boolean)).size

  if (loading) return <div className="public_dashboard_loading">Loading dashboard...</div>
