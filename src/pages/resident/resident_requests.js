            ...
            {requests.map((req) => (
              <li key={req.id} className="rr-request-card">
                <div className="rr-card-header">
                  <span className="rr-category">{req.category || 'Other'}</span>
                  <span className={`rr-status-badge rr-status-badge--${(req.status || 'open').toLowerCase().replace(/\s+/g, '_')}`}>
                    {req.status || 'Open'}
                  </span>
                </div>
                <p className="rr-description">{req.description || 'No description'}</p>
