 ...
 * Returns 'HH:MM' for today, 'Mon DD' for this year, 'DD/MM/YY' for older.
 */
...
export default function WorkerMessages() {
	...
	const [requests_map, set_requests_map] = useState({}) // request_id → request object
	const [conversations, set_conversations] = useState([])
	const [selected_id, set_selected_id] = useState(null)
	const [search, set_search] = useState('')
	const [auth_loading, set_auth_loading] = useState(true)
	...
	useEffect(() => {
		const unsub_auth = onAuthStateChanged(auth, async (user) => {
			...
			try {
				const { worker, requests } = await fetch_worker_dashboard_data(
					user.uid
				)
				set_worker(worker)

				// Build a lookup map so ConversationItem can show request category
				const map = {}
				requests.forEach((r) => {
					map[r.id] = r
				})
				...
				set_error('Failed to load worker data.')
			} finally {
				set_auth_loading(false)
			}
		})
	...
	/* ── Conversations subscription (starts once worker is known) ─────── */
	...
	if (auth_loading) {
		return (
			<div className="wm-fullscreen-state">
	...
	if (error) {
		return (
			<div className="wm-fullscreen-state">
				<p className="wm-error-text">{error}</p>
			</div>
		)
	}
						...
						{!conv_loading && filtered.length === 0 && (
							<div className="wm-conv-empty">
								{search
									...
									: 'No conversations yet. Messages will appear here once a resident contacts you or you initiate from a request.'}
							</div>
						)}
...
function ConversationItem({ conv, req, is_selected, worker_uid, on_click }) {
	const { last_message, unread_count, request_id } = conv
		...
		<div className="wm-empty-thread">
			...
			<p className="wm-empty-thread-sub">
				{has_conversations
					...
					: 'Open a request from your dashboard and send the first message to a resident.'}
			</p>
		</div>
	)
}
