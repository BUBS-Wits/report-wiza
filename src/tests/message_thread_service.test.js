...
import { onSnapshot, addDoc, writeBatch, serverTimestamp, doc } from 'firebase/firestore';
...
describe('Message Thread Service', () => {
    ...
    describe('send_message', () => {
        test('throws an error if message text is empty or just whitespace', async () => {
            await expect(send_message({ text: '   ' })).rejects.toThrow('Message text cannot be empty.');
    ...
    describe('mark_messages_read', () => {
        ...
        test('batches updates only for unread messages sent to the current user', async () => {
            ...
            expect(mockBatchUpdate).toHaveBeenCalledWith('messages/msg1', { read: true });
            expect(mockBatchUpdate).toHaveBeenCalledWith('messages/msg2', { read: true });
    ...
    describe('format_message_time', () => {
        ...
        test('returns DD/MMM HH:MM if the timestamp is from a previous day', () => {
            ...
            expect(format_message_time(pastDate)).toMatch(/14 Oct 09:05|Oct 14 09:05/i);
        });
    });
});