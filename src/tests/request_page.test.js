...
jest.mock('../components/nav_bar/nav_bar.js', () => () => <div data-testid="nav-bar" />);
jest.mock('../components/submit_prompt_modal/submit_prompt_modal.js', () => 
    ({ is_open, on_close, on_continue }) => is_open ? (
...
jest.mock('../components/request_form/request_form.js', () => 
    ({ onSubmit, submitting }) => {
...
describe('RequestPage Component', () => {
    ...
    beforeEach(() => {
        ...
        mockRequest = {
            ...
            to_string: jest.fn().mockReturnValue(JSON.stringify({ data: 'mock' }))
        };
    ...
    test('aborts submission if validation fails', async () => {
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith('Location/ward info missing:', mockRequest.loc_info);
    ...
    test('submits successfully when anonymous user clicks "Continue Anonymously" on modal', async () => {
        ...
        expect(global.fetch).toHaveBeenCalledWith('/api/submit-request', expect.objectContaining({
            method: 'POST',
