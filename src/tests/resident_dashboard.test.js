...
describe('ResidentDashboard Component', () => {
    ...
    test('loads and displays resident profile and requests', async () => {
        ...
        const mockRequests = [
            { id: 'req1', category: 'Pothole', status: 'submitted', description: 'Big pothole', created_at: new Date('2023-01-01') },
            { id: 'req2', category: 'Water Leak', status: 'in_progress', description: 'Leaking pipe', created_at: new Date('2023-01-02') }
        ]
    ...
    test('shows empty state when there are no requests', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText("You haven't submitted any service requests yet.")).toBeInTheDocument()
        })
    })
    ...
    test('handles request selection correctly', async () => {
        ...
        const mockProfile = { uid: 'user123', name: 'John Doe' }
        const mockRequests = [
            { id: 'req1', category: 'Pothole', status: 'submitted', description: 'Big pothole', created_at: new Date() },
            { id: 'req2', category: 'Water Leak', status: 'in_progress', description: 'Leaking pipe', created_at: new Date() }
        ]
