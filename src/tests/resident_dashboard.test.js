...
jest.mock('../components/notification_bell/notification_bell', () =>
...
jest.mock('../components/feedback_form/feedback_form', () =>
    function MockFeedbackForm({ onCancel, onSubmit }) {
        return (
            <div data-testid="feedback-form">
                <button onClick={() => onSubmit({ rating: 4, comment: 'Good job' })}>
                    Submit Mock Review
                </button>
...
describe('ResidentDashboard Component', () => {
    ...
    test('shows empty state when there are no requests and handles logout', async () => {
        ...
        fetch_resident_profile.mockResolvedValue({ uid: 'user123', name: 'Jane Doe' })
        fetch_resident_requests.mockResolvedValue([])

        render(<ResidentDashboard />)

        await waitFor(() => {
            expect(screen.getByText('You have not submitted any service requests yet.')).toBeInTheDocument()
        })
    ...
    test('loads and displays resident profile, unread count, and selects first request', async () => {
        ...
        const secondCard = screen.getAllByText('Water Leak')[0].closest('button')
        ...
        expect(screen.getByText('Select a request to view details and messages.')).toBeInTheDocument()
    })
    ...
    test('processes location, fetches signed URL, and handles StarRating', async () => {
        ...
        fetch_resident_profile.mockResolvedValue({ uid: 'user123', name: 'John' })
        ...
        global.fetch.mockResolvedValueOnce({
            ok: true,
            json: async () => ({ data: 'http://signed.com/img.jpg?X-Amz-Date=20260516T095040Z&X-Amz-Expires=432000' })
        })
        ...
        const locLink = screen.getByRole('link', { name: /-26\.230900,\s*28\.058300/i })
        expect(locLink).toBeInTheDocument()

        // Ensure signed URL was fetched
        expect(global.fetch).toHaveBeenCalledWith('/api/get-signed-url?request_uid=req1')
    ...
    test('handles signed URL fetch failure gracefully', async () => {
        ...
        expect(img).toHaveAttribute('src', 'http://expired.com/img.jpg?X-Amz-Date=20000101T000000Z&X-Amz-Expires=3600')
    })
    ...
    test('submits feedback successfully with auth', async () => {
        ...
        fetch_resident_requests.mockResolvedValue([{ id: 'req1', status: 'resolved' }])

        // Mock auth token
        auth.currentUser = { getIdToken: jest.fn().mockResolvedValue('mock_token') }
        ...
        await waitFor(() => {
            expect(global.fetch).toHaveBeenCalledWith('/api/submit-review', expect.objectContaining({
                headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer mock_token' },
                body: JSON.stringify({ request_uid: 'req1', rating: 4, comment: 'Good job' })
            }))
            expect(window.alert).toHaveBeenCalledWith('Review successfully submitted.')
        })
    })
    ...
    test('handles feedback API generic text error gracefully', async () => {
        ...
        await waitFor(() => {
            expect(window.alert).toHaveBeenCalledWith('Server error. Please check console for details.')
        })
    })
    ...
    test('handles close reason fetch failure', async () => {
        ...
        fetch_resident_profile.mockResolvedValue({ uid: 'user123' })
        fetch_resident_requests.mockResolvedValue([{ id: 'req1', status: 'closed' }])
