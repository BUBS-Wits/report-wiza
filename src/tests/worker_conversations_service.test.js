...
describe('Worker Conversations Service', () => {
    ...
    test('sets up two queries and returns a combined unsubscribe function', () => {
        const unsub = subscribe_to_worker_conversations('worker_123', mockOnUpdate, mockOnError);
    ...
    test('merges sent and received messages, groups by request, and calculates unread count', () => {
        subscribe_to_worker_conversations('worker_123', mockOnUpdate, mockOnError);
        ...
        sentCallback(createMockSnap([
            { id: 'msg_1', request_id: 'req_A', sender_uid: 'worker_123', receiver_uid: 'resident_1', text: 'Hello', sent_at: mockTimestamp(1000), read: true }
        ]));

        // Simulate Worker receiving 2 messages in Request A (1 unread), and 1 in Request B (unread)
        receivedCallback(createMockSnap([
            { id: 'msg_2', request_id: 'req_A', sender_uid: 'resident_1', receiver_uid: 'worker_123', text: 'Hi back', sent_at: mockTimestamp(2000), read: true },
            { id: 'msg_3', request_id: 'req_A', sender_uid: 'resident_1', receiver_uid: 'worker_123', text: 'Are you there?', sent_at: mockTimestamp(3000), read: false },
            { id: 'msg_4', request_id: 'req_B', sender_uid: 'resident_2', receiver_uid: 'worker_123', text: 'Help', sent_at: mockTimestamp(4000), read: false }
        ]));
        ...
        const conversations = mockOnUpdate.mock.calls[mockOnUpdate.mock.calls.length - 1][0];
    ...
    test('deduplicates messages if they somehow appear in both snapshots', () => {
        ...
        const dupMessage = { id: 'msg_X', request_id: 'req_Z', sender_uid: 'worker_123', receiver_uid: 'resident_1', sent_at: mockTimestamp(100) };
