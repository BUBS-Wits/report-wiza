...
jest.mock('../components/message_thread/message_thread.js', () => () => (
    <div data-testid="mock-message-thread">Message Thread</div>
))
...
describe('WorkerDashboard component', () => {
    ...
    test('clicking a request row opens the detail panel and renders MessageThread', async () => {
        ...
        expect(within(panel).getByText('Large pothole on Main Rd')).toBeInTheDocument()
    ...
    test('shows fallback message when a request has no resident attached', async () => {
        ...
        expect(screen.getByText(/Resident information unavailable/i)).toBeInTheDocument()
        expect(screen.queryByTestId('mock-message-thread')).not.toBeInTheDocument()
    })
    ...
    test('clicking the close button dismisses the panel', async () => {
        ...
        fireEvent.click(screen.getByText('REQ-001').closest('.wd-req-row'))
        expect(screen.getByLabelText('Request detail and messaging')).toBeInTheDocument()
        ...
        await waitFor(() => {
            expect(screen.queryByLabelText('Request detail and messaging')).not.toBeInTheDocument()
        })
    })
