...
jest.mock('../components/worker_nav_bar/worker_nav_bar.js', () => () => <div data-testid="worker-nav" />);
jest.mock('../components/message_thread/message_thread.js', () => ({ request_id }) => (
    <div data-testid="message-thread">Thread: {request_id}</div>
));
...
describe('WorkerMessages Component', () => {
    ...
    beforeEach(() => {
        ...
        subscribe_to_worker_conversations.mockImplementation((uid, callback) => {
            callback([
                {
                    request_id: 'req_1',
                    category: 'Pothole',
    ...
    test('selects a conversation and renders the MessageThread', async () => {
        ...
        expect(screen.getByTestId('message-thread')).toHaveTextContent('Thread: req_1');
    });
